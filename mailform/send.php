...
########################################################################
/*
	send.php バージョン 6.0.1
    ・スパムメールブロックの認証キーワード機能付
    　※認証機能設置していない場合でもデフォルト認証「Ros-mNyan52E」で認証することできます。
    　※(デフォルト認証含め)認証機能を利用しない場合は、認証キーワード（$conf['kwd']）を空白にしてください。
    ・認証用ドメイン
    　※対象のドメイン以外からのメール送信は行わないようにすることができます。
    　※認証機能を利用しない場合は、認証キーワード（$conf['domain']）を空白にしてください。
	・項目が空の場合は、送信するメールで項目を表示しない
    　※注文フォームなどで、入力・選択しないフォームが多い場合にご使用下さい。
    ・問い合わせ先情報のメール本文に載せる/載せない
    　※送信元情報の「IP」、「サーバ情報」、「ホスト名」などの情報を非表示にしたい場合はfalseにしてください。
*/
########################################################################
...
#認証キーワード ※デフォルトは「Ros-mNyan52E」で設定してください。
$conf['kwd'] = '琉新';
...
$conf['mailto'] = 'chocolatemoose1206@gmail.com';

#送信完了時にリダイレクトするサンクスページ
$conf['thanks'] = '../thanks.php';

#設置者に届くメールの件名
$conf['subject'] = 'フォームよりお問い合わせ';
...
TEL:098-851-8380 FAX:098-851-8381
━━━━━━━━━━━━━━━━━━━━━━━━━━
EOM;
...
function send_mail() {
	...
	if(0 < is_array_count($_POST)) {
		...
		// 認証用キーワードがある場合キーワードが一致しているかチェックする
		if("" != $conf['kwd']) {
			// 入力したキーワードが一致していない場合処理を終了する
			if($conf['kwd'] != $_POST["認証"]) {
		...
		foreach($_POST as $key =>  $val) {
			...
			if('0' == $conf['no_disp'] || "" != $val) {
				...
				if(!preg_match("|confirm_.*?|s", $key) &&  '認証' != $key) {
					$cons = '';
		...
		if (null != $_POST["email"] && "" != $_POST["email"]) {
			$admin_mailer->addReplyTo($_POST["email"]);
		}
        ...
        if("" != $_POST['email']) {
            ...
            // 宛先設定
            $user_mailer->AddAddress($_POST['email']);
